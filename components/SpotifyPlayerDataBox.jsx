  ...
  const fetchPlayerData = async (access_token, refresh_token) => {
    try {
      ...
      setPlayerData({
        ...
        album: { name: data.item.album.name, url: data.item.album.external_urls.spotify },
  ...
  return (
    <div className='flex space-x-4 w-2/3 h-1/2 hover:scale-105 transition-all duration-500 ease-in-out cursor-pointer'>
      <div className='w-1/3 h-full border-2 rounded-sm border-[#191414]'>
        {playerData.imageUrl ? (
          <Image className="w-full h-full" src={playerData.imageUrl} width={640} height={640} />
        ) : (
          <AlbumImageErrorText />
        )}
      </div>
          ...
          {Object.keys(playerData).length !== 0 ? (
            <>
              <div className='h-2/3 p-8 flex-col items-center'>
                ...
                <div className="flex space-x-2">
                  <Link href={playerData.album.url}>
                    <h2 className='text-lg font-light italic'>{playerData.album.name}</h2>
                  </Link>
              ...
              <div className='h-1/3 px-8 py-2'>
                <SpotifyPlayhead progress={playerData.progress} duration={playerData.duration} />
              </div>
            </>
          ) : (
